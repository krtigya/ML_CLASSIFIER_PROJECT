...
Additionally, the project includes a robust preprocessing pipeline that filters out Nepali or Hindi (Devanagari) text, removes emojis, punctuations, and stopwords, and performs tokenization and lemmatization to ensure clean and consistent input for the model. A SQLite database is used to store both raw and processed data efficiently.

The final trained model is tested on a separate dataset of MyWorldLink app reviews to evaluate its real-world performance. The system can be used to monitor customer satisfaction, identify key issues, and improve overall service quality through data-driven insights.# Sentiment & Suggestion Classification Project
